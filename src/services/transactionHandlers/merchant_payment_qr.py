"""
src/services/transactionHandlers/merchant_payment_qr.py

Transaction Type #5: Merchant Payment (QR)
Journal Pattern: Debit Customer Wallet (amount+fee+tax) / Credit Merchant
Settlement (Pending) (amount) / Credit Transaction Fee Revenue (fee) /
Credit Tax Collected at Source (tax)
Key Validation: merchant active; transaction limits (stubbed for Phase 1).
"""
...
from src.validators.schemas import MerchantQRPaymentRequest
...
def _handler_factory(request: MerchantQRPaymentRequest):
    def handler(db: Session, transaction: Transaction) -> None:
        ...
        lines = [
            LineInput(
                ...
                narrative=f"Merchant QR payment to {request.merchant_code}",
            ),
            LineInput(
        ...
        if request.fee_amount > 0:
            lines.append(LineInput(
                ...
                currency=request.currency,
                narrative="Merchant QR transaction fee",
            ))
        ...
        create_journal_entry(
            ...
            lines=lines,
            reference_type="MERCHANT_QR_PAYMENT",
...
def process_merchant_qr_payment(db: Session, request: MerchantQRPaymentRequest) -> Transaction:
    return execute_transaction(
        db=db,
        transaction_type=TransactionType.MERCHANT_QR_PAYMENT,
        endpoint="/api/v1/merchant-payment/qr",
